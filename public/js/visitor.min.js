	...
	*/
	 ...
	 $('#mem-user-signup-form').validate({
	 	  submitHandler: function(form, event) { //Send signup data
		    ...
		    $.ajax({
	            url: "http://localhost:8000/webservices/create-account",
	            type: "POST",
	            	...
	            	function(json_response){	            			            			            			            
	            		switch(json_response.code){
	            			case 201: 
	            				...
	            				$("#signup-signin-messages").html(json_response.message);//display message about the email 
	            			case 202: 
	            				$("#singup-div-form").hide();
	            				$("#singin-div-form").show();	         
	            				$("#signup-link").removeClass('active');
	 							$("#signin-link").addClass('active');				
	            				$("#signup-signin-messages").html(json_response.message);//display message about email already exist
	            		}	                	
	               	}//end function	                    				
			});//end ajax
		  }//end submithandler
	...
	//validates signin form
	 ...
	 $('#mem-user-signin-form').validate({
	 	submitHandler: function(form, event) { //Send signup data
	 	  	event.preventDefault();	 	  	
		    $.ajax({
	            url: "http://localhost:8000/webservices/validate-token",
	            type: "POST",
	            ...
	            success: 
	            	function(json_response){	            			            			            			            
	            		switch(json_response.code){
	            			case 200: 
	            				window.location="http://localhost:8000/dashboard/"+json_response.session_id;	            					            				
	            			case 400: 	            				
	            				$("#singin-div-form").show();
	            				$("#signup-signin-messages").html(json_response.message);//display message about wrong login data
	            		}	                	
	               	}//end function	                    				
			});//end ajax
		  }//end submithandler
